...
type SakuratoTeamParser struct {
...
func (p *SakuratoTeamParser) Parse(entry *dirinfo.Entry) error {
...
func (p *SakuratoTeamParser) regexMatchPredefined(name string) (*rawInfo, error) {
...
var (
	regexFileName = regexp.MustCompile(`^\[Sakurato\] (?P<name>.*) \[(?P<epnum>\d+)\]`)
)

...
func (p *NekomoeTeamParser) Parse(entry *dirinfo.Entry) error {
...
func (p *NekomoeTeamParser) regexMatchPredefined(name string) (*rawInfo, error) {
...
var (
	regexFileName = regexp.MustCompile(`^\[Nekomoe kissaten\]\[(?P<name>.*)\]\[(?P<epnum>\d+)\]`)
)
